...
public class WebcConfig {
	...
	public static class FilterInitParam {
		...
		public boolean csrfOff; // 是否关闭csrf. @Since 1.2.0
	...
	public static FilterInitParam decodeFilterInitParam(javax.servlet.FilterConfig filterConfig) {
		...
		ret.asyncOff = getBooleanParam(filterConfig, Props.asyncOff, false);
		...
		ret.csrfOff = getBooleanParam(filterConfig, Props.csrfOff, false);
	...
	public static void encodeFilterInitParam(FilterRegistration.Dynamic dynamic, FilterInitParam param) {
		...
		if (param.csrfOff) {
			dynamic.setInitParameter(Props.csrfOff, String.valueOf(param.csrfOff));
		}
